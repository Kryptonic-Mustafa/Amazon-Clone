...
FILES_TO_UPDATE = {
    # ==========================================
    # 1. PRODUCT CARD (Strict Sale Flag Logic)
    # ==========================================
...
export default function ProductCard({ product }: { product: any }) {
  ...
  // --- STRICT SALE LOGIC ---
  // Only apply sale visuals if the database EXPLICITLY says sale_flag is 1
  ...
  return (
    <div className="group relative bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col h-full">
      ...
      {/* TOP LEFT: Discount Percentage Badge (STRICTLY IF ON SALE) */}
      {isSale && discount > 0 && (
      ...
      {/* TOP RIGHT: SALE Badge (STRICTLY IF ON SALE) */}
      {isSale && (
...
export default function FilterSidebar({
  ...
  return (
    <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-6 sticky top-24 z-20">
      
      {/* Back to Home Button - Flush to top now */}
      <Link 
        href="/" 
...
""",
    ...
    # ==========================================
    # 3. SHOP PAGE (Sticky Header added)
    # ==========================================
    ...
    <div className="bg-slate-50 min-h-screen flex flex-col">
      <main className="flex-grow container mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 relative">
          ...
          {/* STICKY TOP BAR: Title & Dropdown */}
          {/* Note: top-24 makes it sit perfectly below your main navbar while scrolling */}
          <div className="sticky top-24 z-30 flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 bg-white/80 backdrop-blur-xl p-4 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="relative w-full sm:w-auto">
              <select 
                ...
                className="w-full sm:w-auto border border-slate-300 rounded-xl pl-4 pr-10 py-2.5 text-sm font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer bg-white hover:bg-slate-50 transition-colors appearance-none shadow-sm"
              >
...
def apply_shop_fixes():
    print("Applying Final Polish to Shop UI...")
    ...
    print("\n🎉 DONE! The Shop Products header is now sticky, the sidebar top-spacing is fixed, and SALE badges strictly obey the database sale_flag.")
